...
// src/index.ts
var DEFAULT_FAVICON_URL = "favicon.ico";
var UAT_FAVICON_URL = "assets/favicon_uat.ico";
...
var renderAltair = (options = {}) => {
  ...
  if (options.serveInitialOptionsInSeperateRequest) {
    if (!options.cspNonce) {
      ...
      return altairHtml.replace(
        "</body>",
        () => {
          var _a;
          return `<script type="module" nonce="${(_a = options.cspNonce) != null ? _a : ""}" src="${sanitizeAttributeValue(scriptName)}"><\/script></body>`;
        }
      );
    }
  }
  return altairHtml.replace(
    ...
    () => {
      var _a;
      return `<script type="module" nonce="${(_a = options.cspNonce) != null ? _a : ""}">${initialOptions}<\/script></body>`;
    }
  );
};
var getRenderedAltairHtml = (options) => {
  var _a;
  const baseURL = options.baseURL || "./";
  let renderedHtml = getAltairHtml().replace(/<base.*>/, `<base href="${baseURL}">`).replace("<style>", `<style nonce="${(_a = options.cspNonce) != null ? _a : ""}">`);
...
var getFaviconURL = (options) => {
  var _a, _b;
  if (options.faviconURL) {
    return options.faviconURL;
  }
  const environment = (_b = (_a = options.initialSettings) == null ? void 0 : _a.environment) == null ? void 0 : _b.toString().trim().toLowerCase();
  return environment === "uat" ? UAT_FAVICON_URL : DEFAULT_FAVICON_URL;
};
var sanitizeAttributeValue = (value) => {
